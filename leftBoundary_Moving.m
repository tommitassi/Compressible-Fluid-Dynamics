...
%% 3. Creation of the mesh relative to the left moving boundary

function [ x, u ] = leftBoundary_Moving( x_in, u_in, ub, type, x_f, v0, c )

% leftBoundary permits to complete the mesh using the left
% boundary condition and to compute the solution in those points
% the boundary moves with a constant decelleration
...
%   x_f     [1 x 1]         x breaking distance, from the initial point
%   v0      [1 x 1]         Initial velocity of the boundary
...
A = - v0^2 / (4 * x_f); % parabola: x = A*x^2 + v0*x
